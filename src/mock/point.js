...
function getPoints(amount) {
  return Array.from({ length: amount }, (value, id) => {
    ...
    return {
      ...
      'date_from': (new Date(Date.now() + getRandomNumber(0, 1000 * 60 * 60 * 24))).toISOString(),
      'date_to': (new Date(Date.now() + getRandomNumber(1000 * 60 * 60 * 24, 1000 * 60 * 60 * 24 * 3))).toISOString(),
